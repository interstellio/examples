"""An example RADIUS backend for lunar, built with FastAPI.
...
# The FastAPI ASGI application. uvicorn serves it as ``app:application``; the
# view functions below register their routes on it with @application.get /
# @application.post.
application = FastAPI()
...
async def auth(server_id: str, virtual_id: str, request: Request):
    ...
    try:
        packet = json.loads(await request.body() or b"{}")
    except ValueError:
        packet = {}
    ...
    return Response(content=json.dumps(reply),
                    media_type="application/json")
...
async def coa(server_id: str, virtual_id: str, request: Request):
    ...
    """
    try:
        request_packet = json.loads(await request.body() or b"{}")
    except ValueError:
        request_packet = {}
