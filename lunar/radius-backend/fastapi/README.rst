==========================================
Example RADIUS backend for Lunar (FastAPI)
==========================================

**This example assumes you are a Python developer with a working knowledge of
REST APIs, async Python, the FastAPI framework, the uvicorn server and a web
server like nginx.**

A minimal, working example of a **lunar backend** written in Python with the
`FastAPI <https://fastapi.tiangolo.com/>`_ web framework. It implements the same
...
* ``fastapi`` (the web framework, an ASGI / async framework)
...
worker, which is fine for local testing and trying things out.
``run.py`` will also tell you (and point you back here) if any dependency is
missing.
...
* ``app.py`` - the FastAPI application and routes, plus the hard-coded ``TEST``
  virtual returned to lunar. Each route is a small async view function
  decorated with ``@application.get`` / ``@application.post``; the whole request
  flow (read the packet, detect the method, verify it, build the reply) lives
  inline in the ``auth`` view so it reads top to bottom. PAP is a one-line
  comparison, so it is done inline; the other methods call the modules below.
