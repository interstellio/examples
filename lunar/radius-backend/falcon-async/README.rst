...
* ``falcon`` (the web framework, used in its async / ASGI mode)
* ``cryptography`` (the DES primitive used by MS-CHAP / MS-CHAPv2)
* ``uvicorn`` (the ASGI server ``run.py`` uses for the fast, concurrent demo)
* ``uvloop`` (a faster event loop uvicorn uses automatically when installed)
...
It listens on ``http://127.0.0.1:5555``. ``app.py`` starts a single uvicorn
worker, which is fine for local testing and trying things out. ``run.py`` will
...
For anything beyond local testing, run it under several workers and put it
behind a reverse proxy:

* **Use the bundled** ``run.py`` - the quickest way to demo it fast. It detects
  how many processors the box has and starts uvicorn with several worker
  processes on the fast uvloop event loop, so the many concurrent connections
  lunar opens are handled in parallel:
  ...
  It sizes the pool automatically (``2 x cores + 1`` uvicorn workers on uvloop)
  and listens on ``127.0.0.1:5555`` - there is nothing to configure.

  Pass ``--nodebug`` to quiet both this backend and uvicorn down to errors and
  critical only:
...
* **Or run uvicorn directly** if you want to tune the flags yourself:

  .. code:: bash

      uvicorn --workers 8 --loop uvloop \
          --host 127.0.0.1 --port 5555 app:application

  Tune ``--workers`` to your CPU (a common start is ``2 x cores + 1``).

* **Put nginx in front** of uvicorn for TLS termination, connection keep-alive
  ...
  and load-balancing across uvicorn workers (or several backend hosts) is how
...
Why async is no faster here
===========================

You will find this async version runs at essentially the SAME speed as the
plain (WSGI) ``falcon`` example - that is expected. Async only wins when a
request spends time waiting on I/O it can ``await`` (a database query, a
directory or upstream API call), so the event loop can serve other requests on
the same worker meanwhile. This example does none of that: it authenticates
``test`` / ``test`` with in-memory CPU work and hands back a static IP, so there
is nothing to await. CPU-bound work does not overlap under async (one event
loop, one handler at a time per worker), and both versions get their real
parallelism the same way - from multiple worker processes.

Async pays off once you give it real I/O to wait on. Swap the hard-coded check
for a database, directory or API lookup (see `Make it your own`_) and one
uvicorn worker can keep many such requests in flight at once, handling more
concurrency with fewer processes and less memory than the thread-per-request
WSGI model. With a static answer, the two are tied.
...
* ``app.py`` - the Falcon ASGI application and routes, plus the hard-coded
  ``TEST`` virtual returned to lunar. Each route is a small resource class with
  ``async`` responders; the whole request flow (read the packet, detect the
  method, verify it, build the reply) lives inline in ``AuthResource.on_post``
